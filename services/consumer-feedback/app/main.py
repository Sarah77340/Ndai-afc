...
def detect_language(text: str) -> str:
    ...
    try:
        ...
        if lang not in SUPPORTED_LANGS:
            logger.warning("⚠️ detected lang=%s not supported -> fallback en", lang)
            return "en"
        return lang
    except Exception as e:
        logger.warning("⚠️ lang detect failed -> fallback en: %s", e)
        return "en"
...
def translate_to_en(text: str, source_lang: str) -> Tuple[Optional[str], bool]:
    ...
    for attempt in range(1, 4):
        try:
            ...
            logger.warning("⚠️ translate failed attempt %s/3 (lang=%s): %s", attempt, source_lang, e)
    ...
    try:
        while True:
            ...
            try:
                ...
                logger.warning("⚠️ non-JSON skipped: %r", raw[:200])
                continue
